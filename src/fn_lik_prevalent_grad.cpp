...
arma::mat fn_lik_prevalent_grad(arma::vec u_ID, arma::vec v_ID, arma::mat X_ID, arma::mat Qe, arma::vec cen, double gamma_all, 
                                  double eta, double k, arma::vec px, arma::vec pw, arma::vec px21, arma::vec px22, arma::vec pw2, int len_par) {
  ...
  arma::mat res(nID, len_par, fill::zeros);
  ...
  for (int g = 0; g < nID; ++g) {
    ...
    for (int j = 0; j < ns; ++j) {
      ...
      if(ceni == 1) {
        ...
        ga_h = eta * k * pow(ui - si, k - 1);
        dga_h[len_par - 2] = k * pow(ui - si, k - 1);
        dga_h[len_par - 1] = eta * pow(ui - si, k - 1) * (1 + k * log(ui - si));
        
        ga_S = exp(- eta * pow(ui - si, k));
        dga_S[len_par - 2] = -ga_S * pow(ui - si, k);
        dga_S[len_par - 1] = -ga_S * eta * pow(ui - si, k) * log(ui - si);
    ...
    for (int jj = 0; jj < nas; ++jj) {
      ...
      ga_h = eta * k * pow(as1[jj], k - 1);
      dga_h[len_par - 2] = k * pow(as1[jj], k - 1);
      dga_h[len_par - 1] = eta * pow(as1[jj], k - 1) * (1 + k * log(as1[jj]));
      
      ga_S = exp(- eta * pow(as1[jj], k));
      dga_S[len_par - 2] = -ga_S * pow(as1[jj], k);
      dga_S[len_par - 1] = -ga_S * eta * pow(as1[jj], k) * log(as1[jj]);
    ...
    for (int g2 = 0; g2 < len_par; ++g2) {
      res(g, g2) = dnum_part[g2]/num_part - dden_part[g2]/den_part;
    }
