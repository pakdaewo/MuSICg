...
double fn_lik_prevalent_i(int id, arma::vec u_ID, arma::vec v_ID, arma::mat Qe, arma::vec cen, double gamma_all, 
                          double eta, double k, arma::vec px, arma::vec pw, arma::vec px21, arma::vec px22, arma::vec pw2) {
  ...
  for (int i = 0; i < ns; ++i) {
    ...
    ga = eta * k * pow(ui - s[i], k - 1) * exp(-eta * pow(ui - s[i], k));
  ...
  for (int j = 0; j < nas; ++j) {
    ...
    ga_den = eta * k * pow(as1[j], k - 1) * exp(-eta * pow(as1[j], k));
