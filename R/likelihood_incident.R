likelihood_incident <- function(par_coef, ind_coef, dataset_incident, cl, ...) {
  ...
  if (is.null(cl)) {
    ...
    loglik <- fun_lik_incident(unique_ID, ID, visit, state, Qe, X_ID, cen, gamma_all)
  } else {
...
# system.time(likelihood_incident(par_coef, ind_coef, dataset_incident, cl=cl))
