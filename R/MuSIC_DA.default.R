MuSIC_DA.default <- function(dataset_incident, dataset_prevalent, xw_rule, cl = NULL, optim.method = "nlminb", se = FALSE, ...) {
  ...
  MLE_trunc <- optimf(c(0,0), likelihood_prevalent_DA, likelihood_prevalent_grad_DA,
                      par_coef = est0, ind_coef = ind_coef, dataset_prevalent = dataset_prevalent,
                      xw_rule = xw_rule, cl = cl, only.trunc = TRUE, control = list(maxit = 100000, trace = T))
  ...
  while ((distance > tol) & (ntry < 100)) {
    ...
    MLE_all <- optimf(old_est0, likelihood_DA, likelihood_grad_DA, ind_coef = ind_coef,
                      par_trunc = old_trunc, dataset_incident = dataset_incident, dataset_prevalent = dataset_prevalent,
                      xw_rule = xw_rule, cl = cl, control = list(maxit = 100000, trace = T))

    new_est0 <- MLE_all$par

    MLE_trunc <- optimf(old_trunc, likelihood_prevalent_DA, likelihood_prevalent_grad_DA,
  ...
  # MLE_all <- optimf(c(est0, est0_trunc), likelihood_forhess_DA, likelihood_forhess_grad_DA, ind_coef = ind_coef,
  #                   par_trunc = old_trunc, dataset_incident = dataset_incident, dataset_prevalent = dataset_prevalent,
  #                   xw_rule = xw_rule, cl = cl, control = list(maxit = 100000, trace = T))
  # 
  # hessian
  if (se == TRUE) {
    hessian_h <- numDeriv::hessian(func = likelihood_forhess_DA, x = old_all, ind_coef = ind_coef,
    ...
    cross_i <- likelihood_forhess_grad_DA(old_all, ind_coef, dataset_incident, dataset_prevalent, xw_rule, cl, csum = FALSE)
