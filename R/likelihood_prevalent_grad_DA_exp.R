likelihood_prevalent_grad_DA_exp <- function(par_trunc, par_coef, ind_coef, dataset_prevalent, xw_rule, cl, csum = TRUE, only.trunc = FALSE, ...) {
  ...
  if (is.null(cl)) {
    log_trans_prob <- t(apply(target, 1, fn_lik_prevalent_grad_i_exp, u_ID = u_ID, v_ID = v_ID, X_ID = X_ID, Qe = Qe, cen = cen, gamma_all = gamma_all, 
                            eta = eta, px = px, pw = pw, px21 = px2[,1], px22 = px2[,2], pw2 = pw2, len_par = len_par))
  } else {
    if (.Platform$OS.type != "unix") {
      parallel_fun <- if (isTRUE(getOption("pboptions")$use_lb)) parLapplyLB else parLapply
      collected <- parallel_fun(cl = cl, l_target, fn_lik_prevalent_grad_i_exp, u_ID = u_ID, v_ID = v_ID, X_ID = X_ID, Qe = Qe, cen = cen, gamma_all = gamma_all, 
                                eta = eta, px = px, pw = pw, px21 = px2[,1], px22 = px2[,2], pw2 = pw2, len_par = len_par)
    } else {
      collected <- mclapply(l_target, fn_lik_prevalent_grad_i_exp, u_ID = u_ID, v_ID = v_ID, X_ID = X_ID, Qe = Qe, cen = cen, gamma_all = gamma_all, 
                            eta = eta, px = px, pw = pw, px21 = px2[,1], px22 = px2[,2], pw2 = pw2, len_par = len_par, mc.cores = length(cl))
    }
    log_trans_prob <- t(sapply(collected, function(x) x))
  }
  
  log_trans_prob[,7] <- log_trans_prob[,7] * gamma_all
  log_trans_prob[,len_par] <- log_trans_prob[,len_par] * eta
  ...
  if (only.trunc == TRUE) {
    if (csum == TRUE)  d_loglik <- colSums(log_trans_prob)[len_par]
    if (csum == FALSE) d_loglik <- log_trans_prob[, len_par]
  }
  return(-d_loglik)
