likelihood_prevalent <- function(par_trunc, par_coef, ind_coef, dataset_prevalent, xw_rule, cl, ...) {
  ...
  if (is.null(cl)) {
    ...
    loglik_pre <- fn_lik_prevalent(u_ID, v_ID, Qe, cen, gamma_all, eta, k, px, pw, px2[,1], px2[,2], pw2)
  } else {
...
# system.time(likelihood_prevalent(par_trunc, par_coef, ind_coef, dataset_prevalent, xw_rule, cl))
# system.time(likelihood_prevalent(par_trunc, par_coef, ind_coef, dataset_prevalent, xw_rule, NULL))


# system.time(fn_lik_prevalent(u_ID, v_ID, Qe, cen, gamma_all, eta, k, px, pw, px2[,1], px2[,2], pw2))
# 
# system.time(lognum_den <- apply(target, 1, fn_lik_prevalent_i, u_ID = u_ID, v_ID = v_ID, Qe = Qe, cen = cen, gamma_all = gamma_all, 
#                     eta = eta, k = k, px = px, pw = pw, px21 = px2[,1], px22 = px2[,2], pw2 = pw2))

# tt <- function(x, ...) {
#   par_coef <- x[1:19]
#   par_trunc <- x[20:21]
#   likelihood_prevalent(par_trunc, par_coef, ind_coef, dataset_prevalent, xw_rule, cl)
# }
# # l <- likelihood_prevalent(par_trunc, par_coef, ind_coef, dataset_prevalent, xw_rule, cl)
# g <- likelihood_prevalent_grad(par_trunc, par_coef, ind_coef, dataset_prevalent, xw_rule, cl)
# gn <- grad(func = likelihood_prevalent, par_trunc, par_coef = par_coef, ind_coef = ind_coef, dataset_prevalent = dataset_prevalent,
