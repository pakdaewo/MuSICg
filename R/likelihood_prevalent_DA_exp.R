likelihood_prevalent_DA_exp <- function(par_trunc, par_coef, ind_coef, dataset_prevalent, xw_rule, cl, ...) {
  ...
  if (is.null(cl)) {
    lognum_den <- apply(target, 1, fn_lik_prevalent_i_exp, u_ID = u_ID, v_ID = v_ID, Qe = Qe, cen = cen, gamma_all = gamma_all, 
                 eta = eta, px = px, pw = pw, px21 = px2[,1], px22 = px2[,2], pw2 = pw2)
  } else {
    if (.Platform$OS.type != "unix") {
      parallel_fun <- if (isTRUE(getOption("pboptions")$use_lb)) parLapplyLB else parLapply
      collected <- parallel_fun(cl = cl, l_target, fn_lik_prevalent_i_exp, u_ID = u_ID, v_ID = v_ID, Qe = Qe, cen = cen, gamma_all = gamma_all, 
                                eta = eta, px = px, pw = pw, px21 = px2[,1], px22 = px2[,2], pw2 = pw2)
    } else {
      collected <- mclapply(l_target,  fn_lik_prevalent_i_exp, u_ID = u_ID, v_ID = v_ID, Qe = Qe, cen = cen, gamma_all = gamma_all, 
                            eta = eta, px = px, pw = pw, px21 = px2[,1], px22 = px2[,2], pw2 = pw2, mc.cores = length(cl))
    }
...
# g <- likelihood_prevalent_grad_DA_exp(par_trunc, par_coef, ind_coef, dataset_prevalent, xw_rule, cl)
# gn <- grad(func = likelihood_prevalent_DA_exp, par_trunc, par_coef = par_coef, ind_coef = ind_coef, dataset_prevalent = dataset_prevalent,
