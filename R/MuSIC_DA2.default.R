MuSIC_DA2.default <- function(dataset_incident, dataset_prevalent, xw_rule, cl = NULL, optim.method = "nlminb", hess = FALSE, ...) {
  ...
  MLE_pall <- nlminb(c(est0, -0.1, -0.1), likelihood_forhess_DA, gradient = likelihood_forhess_grad_DA, ind_coef = ind_coef,
  ...
  # hessian
  if (hess == TRUE) {
    hessian_h <- numDeriv::hessian(func = likelihood_forhess_DA, x = est_all, ind_coef = ind_coef,
                                   dataset_incident = dataset_incident, dataset_prevalent = dataset_prevalent,
                                   dist_trunc = dist_trunc, xw_rule = xw_rule, cl = cl)
    # crossproduct of gradient
    len_par <- length(est_all)
    cross_i <- likelihood_forhess_grad_DA(est_all, ind_coef, dataset_incident, dataset_prevalent, xw_rule, cl, csum = FALSE)
