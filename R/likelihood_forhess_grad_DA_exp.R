likelihood_forhess_grad_DA_exp <- function(est_all, ind_coef, dataset_incident, dataset_prevalent, xw_rule, cl, csum = TRUE, ...) {
  ...
  if (is.null(dataset_prevalent)) {
    ...
    l_prevalent <- likelihood_prevalent_grad_DA_exp(par_trunc, par_coef, ind_coef, dataset_prevalent, xw_rule, cl, csum = csum) 
  }
  
  if (csum == TRUE) lik =  c(l_incident,0) + l_prevalent
  if (csum == FALSE) {
    lik = rbind(cbind(l_incident, 0), l_prevalent)
  } 
